...
/*
var root = $.xmlToJSON(xml_dom_object, {nodeHandler: function(_node) { //_node is a reference to an actual xml node being processed
//Put Code to handle nodes differently
if(_node.nodeName === "foo") { //Handle only "foo" nodes
  return {Text:"", _nodeName:"MyFoo", _processAttributes:false};
//Above line will rename all "foo" nodes to "MyFoo" and skip all attributes

if(_node.nodeName === "unwanted") { //Handle only "unwanted" nodes
  return -1; //This will skip the node entirely and you will not see it in resulting json object
}
//Do nothing, and node stays the same
}});

In addition a new JSON output mode has been added!!! Now you can choose the type of JSON output between complex and lite version. Complex version is what was there before with additional helper methods added, lite version strips all the extras out leaving only data nodes in the JSON output. To choose a version all you have to do is pass it to xmlToJSON method as an optional parameter. I also have added an enum JSONOutputType that contains complex and lite keywords. Here is the example ...

var root = $.xmlToJSON(xml_dom_object, {outputType: $.JSONOutputType.lite}); //Defaults to $.JSONOutputType.complex
...
Stand-alone version of xmlObjectifier
