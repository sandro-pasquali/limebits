 ...
 *
 *  Sandro -- Simply changed references to AXIS-friendly ones, losing the global NW,
 *  and added the 'selector' method extension to prototype.
 */

AXIS.__selector__ = (function(global) {
...
/**
 * Cacheing off by default
 */
 /*
  * As of v.1.2 cacheing is removed, to be replaced as an add-in in a later version
  * 
AXIS.__selector__.setCache(false);

if(AXIS.settings('enableSelectorCache'))
  {
    AXIS.__selector__.setCache(true);
  };
  ...
  func:       function(q,c)
    {
  	  if(q)
  	    {
  	      return AXIS.__selector__.select(q,c || false) || []; 
  	    }
  	  return [];
  	}
});
  ...
  func:       function(e,s)
    {
  	  if(e && s)
  	    {
  	      return AXIS.__selector__.match(e,s); 
  	    }
  	  return false;
  	}
});
...
/**
    ...
    AXIS.__selector__.match(element, selector);

    // return array of elements
    AXIS.__selector__.select(selector, from);
*/